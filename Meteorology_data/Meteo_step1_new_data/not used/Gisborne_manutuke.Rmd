...
```{r}
current.region <- 'Gisborne'
current.station <- 'Matawhero'
...
daily.data.24am <- 
  tbl_df(read_xlsx(file.path(ExpData, "Gisborne All Stations GDD Comparison.xlsx"), 
                   sheet = 'MatawheroDaily data', col_names = F)) %>% 
  slice(-c(1:4)) %>% 
  select(c(1:8))
...
columns <- columns %>% 
  mutate(final.name = case_when(
    ...
    grepl('Max', value) & grepl('Temp', value) ~ "max.temp",
    grepl('Min', value) & grepl('Temp', value) ~ "min.temp",
    grepl('Rainfall', value) ~ "total.rain",
    is.na(value) ~ 'garbage',
    TRUE ~ value))
...
daily.data.24am <- 
  ...
  filter(!is.na(year)) %>% 
  left_join(select(read_csv(file.path(ExpData, "Gisborne.csv")),"year","day", "radn"), by = c('year','day'))
...
complete.daily.data.radiation <-
  read_csv(file.path(ExpData, "Gisborne.csv")) %>% 
