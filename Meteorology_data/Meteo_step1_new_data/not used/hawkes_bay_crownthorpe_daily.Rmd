...
daily.data.24am <- 
  tbl_df(read_xlsx(file.path(ExpData, "Hawke's Bay 4 Stations GDD Comparison.xlsx"), 
                   sheet = 'Crownthorpe', col_names = F)) %>% 
  slice(-c(1:4)) %>% 
  select(c(1:8))
...
daily.data.24am <- 
  ...
  filter(!is.na(year)) %>% 
  left_join(select(read_csv(file.path(ExpData, "Hawkes_Bay.csv")),"year","day", "radn"), by = c('year','day'))
...
complete.daily.data.radiation <-
  read_csv(file.path(ExpData, "Hawkes_Bay.csv")) %>% 
