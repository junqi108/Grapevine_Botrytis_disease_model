...
daily.data.9am <- tbl_df(read_xlsx(file.path(ExpData, 'MRL.xlsx'), sheet = " daily", col_names = F)) %>% 
  slice(-c(1:8)) %>%
  select(1:25) %>% 
...
daily.Ta.24am <- tbl_df(read_csv(file.path(ExpData, 'Marlborough_central_daily24_2019.csv'), col_names = F)) %>% 
  slice(-c(1:7)) %>%
  select(1:9) %>% 
...
daily.hourl.cal <- 
  tbl_df(read_csv(file.path(IntermediateResults, 
                                             'Blenheim_Hourly_MetData.daily.cal.csv'))) %>% 
  mutate(daily.solar.rad.mj.cal = daily.solar.rad.mj.m2) %>% 
  select(year,day, mean.rh, daily.solar.rad.mj.cal)
  
  # daily.hourl.cal %>% filter(year == 2013 & day >=364)
...
complete.daily.data <- 
  ...
  select(-soil.moisture.5.35cm) %>% 
  left_join(select(daily.data.9am, year, day, soil.moisture.5.35cm), by  = c("year", "day"))
    
short_summary(complete.daily.data)  
 
# glimpse(complete.daily.data)
# qplot( daily.solar.rad.mj.m2, daily.solar.rad.mj.cal, data = complete.daily.data)
# tidy(lm(daily.solar.rad.mj.m2~ daily.solar.rad.mj.cal-1, data = complete.daily.data))
...
mean(complete.daily.data$meanta.24, na.rm = T)
output.data <- 
  complete.daily.data %>% 
  select(stn.no.,year, day, total.rain.mm.0.2, maxta.24, minta.24, meanta.24, daily.solar.rad.mj.m2, 
