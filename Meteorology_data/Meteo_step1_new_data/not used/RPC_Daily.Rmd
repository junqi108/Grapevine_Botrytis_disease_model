...
## Load the data
###remove the notation in the first several rows, and reset the column names
```{r read the table, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE}

daily.data.24am <- 
  tbl_df(read_csv(file.path(ExpData, 'Site1004_RPC_Daily.csv'), col_names = F)) %>% 
  slice(-c(1:5)) %>% 
  select(2:12,17:18) %>%
...
###this is to check whether there are missing data or not
...
daily.hourl.cal <- 
  tbl_df(read_csv(file.path(IntermediateResults,                                          'Site1004_RPC_Hourly_MetData_18_19.daily.cal.csv'))) 
...
complete.daily.data <- 
  ...
  select(stn.no.,year,day, max.temp, min.temp, total.rain, mean.wind.speed, relativehumi_avg, vpd)  
...
blenheim.radiation <- 
  tbl_df(read.csv(file.path(IntermediateResults, 
                                             'Blenheim_Daily_MetData_APSIM_input.csv'), header = TRUE)) %>% 
...
output.data <- 
  complete.daily.data.radiation %>% 
  select(stn.no.,year, year, day, total.rain, max.temp, min.temp, daily.solar.rad.mj.m2,
