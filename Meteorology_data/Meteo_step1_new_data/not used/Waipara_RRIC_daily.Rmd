...
daily.data.24am <- 
  tbl_df(read_xlsx(file.path(ExpData, "Waipara GDD.xlsx"), 
                   sheet = 'HarvNZ RRIC', col_names = F)) %>% 
  slice(-c(1:6)) %>% 
  select(c(1:10))
...
columns <- columns %>% 
  mutate(final.name = case_when(
    ...
    grepl('Tmax', value) ~ "max.temp",
    grepl('Tmin', value) ~ "min.temp",
...
daily.data.24am <- 
  ...
  filter(!is.na(year)) %>% 
  left_join(select(read_csv(file.path(ExpData, "Waipara.csv")),"year","day", "radn"), by = c('year','day'))
...
complete.daily.data.radiation <-
  read_csv(file.path(ExpData, "Waipara.csv")) %>% 
