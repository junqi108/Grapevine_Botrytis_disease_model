...
hourly.data.names <- c("Siteid",    "timestamp", "airtemperatu_avg" )

 
 
hourly.data.int <- 
  read_csv(file.path(ExpData, 'Camshorn Hourly new.csv')) 
 names(hourly.data.int)
...
hourly.data.new <- 
 ...
 # mutate_at(vars(relativehumi_avg:windspeed_avg),funs(replace(.,is.na(.),mean(.,na.rm=TRUE))))%>%
  ...
  select(year,day,hour, airtemperatu_avg)
 
names(hourly.data.new) =c("year", "day", "hour", "mean.ta")
...
```{r, fig.width=15, fig.height=25,echo=FALSE}
 ...
 climate.data.daily.cal <-
  hourly.data.new %>% 
  rename(ta = mean.ta) %>% 
  select(year, day, hour,ta) %>% 
  group_by(year, day) %>% 
  summarise_all(list(max=max, min=min), na.rm = T) %>% 
  select(year, day, ta_max, ta_min )
...
<!--                     paste('Camshorn.climate.data.hourly.csv',sep = '')))         -->
