...
# hourly.data.int <- 
#   tbl_df(read_xlsx(file.path(ExpData, 'BRA New.xlsx'), 
#                    sheet = 'BRA_Hourly', col_names = F)) 
hourly.data.int <- 
  read.csv(file.path(ExpData, 'SEA New hourly.csv'), 
         header = F) 
...
hourly.data.daily.cal <-
  ...
  group_by(year,day) %>% 
   summarize_at(c('relativehumi_avg', 'windspeed_avg'),
               mean, na.rm = TRUE) %>% 
