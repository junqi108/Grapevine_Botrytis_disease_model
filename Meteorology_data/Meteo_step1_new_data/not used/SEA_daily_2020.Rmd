...
daily.data.24am.int <- 
  read.csv(file.path(ExpData, 'SEA New daily.csv'), header = T)
...
daily.data.24am <- 
  ...
  select(stn.no. = siteid, year, day, date, 
         ...
         daily.solar.rad.mj.m2 =solar.energy) 
...
complete.daily.24am <- 
  ...
  ungroup() %>% 
  mutate_at('stn.no.',funs(replace(.,is.na(.),mean(.,na.rm=TRUE)))) %>%
  # mutate_if(is.numeric,funs(replace(.,is.nan(.),
  #                                   mean(.,na.rm=TRUE)))) %>% 
  mutate_at('total.rain',funs(replace(.,is.na(.),0))) %>% 
  mutate_at(vars(max.temp:daily.solar.rad.mj.m2),funs(replace(.,is.na(.),mean(.,na.rm=TRUE))))%>%
  ungroup(.) %>% 
...
daily.hourl.cal <- 
  tbl_df(read_csv(file.path(IntermediateResults,                          'Site1003_SEA_Hourly_New.daily.cal.csv'))) %>% 
  select(-wind.m.s)
